...
function MainApp() {
  ...
  const [settingsOpen, setSettingsOpen] = useState(true); // Keep settings open by default
  ...
  const handleFilesAdded = useCallback(async (files: File[]) => {
    ...
    setShowPopup(false); // Hide popup after images are added
  }, []);
  ...
  const handleClearAll = useCallback(() => {
    setImages([]);
    setShowPopup(true); // Show popup again when all images are cleared
  }, []);
  ...
  const handleRemoveImage = useCallback((imageId: string) => {
    setImages(prev => {
      ...
      if (newImages.length === 0) {
        setShowPopup(true); // Show popup again when all images are removed
      }
      return newImages;
    });
  }, []);
  ...
  return (
    <div className={`flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300`}>
      ...
      {showPopup && (
        <Popup 
          message="Please review and adjust compression settings before uploading your images" 
          onClose={closePopup} 
        />
      )}
